...
import com.nebarry.webapp.domain.enumeration.State;
/**
...
public class DvdResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static Dvd createUpdatedEntity() {
        Dvd dvd = new Dvd()
            .name(UPDATED_NAME)
    ...
    public void createDvd() throws Exception {
        ...
        // Create the Dvd
        restDvdMockMvc.perform(post("/api/dvds").with(csrf())
    ...
    public void updateDvd() throws Exception {
        ...
        restDvdMockMvc.perform(put("/api/dvds").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
    ...
    public void deleteDvd() throws Exception {
        ...
        // Delete the dvd
        restDvdMockMvc.perform(delete("/api/dvds/{id}", dvd.getId()).with(csrf())
