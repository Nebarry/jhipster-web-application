...
public class CdResourceIT {
     ...
     * if they test an entity which requires the current entity.
     */
    public static Cd createUpdatedEntity() {
        Cd cd = new Cd()
            .name(UPDATED_NAME)
    ...
    public void createCd() throws Exception {
        ...
        // Create the Cd
        restCdMockMvc.perform(post("/api/cds").with(csrf())
    ...
    public void updateCd() throws Exception {
        ...
        restCdMockMvc.perform(put("/api/cds").with(csrf())
            .contentType(MediaType.APPLICATION_JSON)
    ...
    public void deleteCd() throws Exception {
        ...
        // Delete the cd
        restCdMockMvc.perform(delete("/api/cds/{id}", cd.getId()).with(csrf())
